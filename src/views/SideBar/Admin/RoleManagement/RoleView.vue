...
function syncLinkedPermissions(viewKey: string, value: boolean) {
  const pair = linkedPermissionPairs.find(
    (p) => p.a === viewKey || p.b === viewKey
  );
  if (!pair) return;
  const otherKey = pair.a === viewKey ? pair.b : pair.a;
  const otherPerm = profileViewPermissions.value.find(
    (p) => p.viewKey === otherKey
  );
...
const loadProfiles = async () => {
  ...
  try {
    ...
    tableData.value = response.data.filter(
      (profile: any) => profile.Profile_Name !== "SuperAdmin"
    );
  } catch (error) {
...
const openPermissionsDialog = async (profile: any) => {
  if (profile.Profile_Name === "Administrador") {
    toast.add({
      ...
      detail:
        "El rol Administrador tiene acceso a todas las vistas y sus permisos no se pueden modificar.",
      life: 5000,
    });
    return;
  }
  ...
  try {
    const response = await axios.get(
      `${API}/profile/auth/${profile.Profile_ID}`
    );
    const permissionsFromApi = response.data as Array<{
      View_Name: string;
      Has_Permission: boolean;
    }>;

    profileViewPermissions.value = APP_VIEWS.map((appView) => {
      const permFromApi = permissionsFromApi.find(
        (p) => p.View_Name === appView.key
      );
      return {
        viewKey: appView.key,
    ...
    console.error(
      `Error cargando permisos para el perfil ${profile.Profile_Name}:`,
      error
    );
    toast.add({
      severity: "error",
      summary: "Error",
      detail: "No se pudieron cargar los permisos del perfil.",
      life: 3000,
    });
    profileViewPermissions.value = APP_VIEWS.map((appView) => ({
      viewKey: appView.key,
...
const saveProfilePermissions = async () => {
  if (
    !selectedProfileForPermissions.value ||
    !selectedProfileForPermissions.value.Profile_ID
  ) {
    toast.add({
      severity: "error",
      summary: "Error",
...
const getEnabledPermissionsCount = (section: any) => {
  ...
  return viewsInSection.filter(
    (view) =>
      profileViewPermissions.value.find((p) => p.viewKey === view.key)
        ?.isEnabled
  ).length;
};
  ...
  <div class="card">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="test-lg text-surface-600 dark:text-surface-300 mt-2">
      ...
      <Button
        ...
        class="p-button-sm"
        @click="
          $router.push({ name: 'Parameter', query: { tabla: 'Profiles' } })
        "
      />
    </div>
        ...
        >
          <template #header>
            <div class="flex justify-between items-center py-2">
              <h2
                class="text-xl font-semibold text-surface-800 dark:text-surface-200"
              >
            ...
            <template #body="slotProps">
              <span v-if="col.type === 'boolean'">
                <Tag
                  ...
                  :icon="
                    slotProps.data[col.field] ? 'pi pi-check' : 'pi pi-times'
                  "
                />
              </span>
    ...
    <Dialog
      ...
      :header="`Gestionar Permisos - ${
        selectedProfileForPermissions?.Profile_Name || ''
      }`"
        ...
        <ScrollPanel style="width: 100%; height: 70vh" class="custombar">
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 p-4">
            <Card
              ...
              :key="section.name"
              :class="[
                'mb-6 rounded-xl shadow-lg',
                isDarkTheme ? 'border border-surface-100 bg-white' : '',
              ]"
            >
              <template #header>
                <div
                ...
                >
                  <div class="flex items-center gap-4">
                    <div
                      :class="`w-12 h-12 rounded-full flex items-center justify-center bg-${section.color}-100 dark:bg-${section.color}-900/20 shadow-md`"
                    >
                      <i
                        :class="[
                          section.icon,
                          `text-xl text-${section.color}-600 dark:text-${section.color}-400`,
                        ]"
                      ></i>
                    </div>
                    <div>
                      <h4
                        :class="[
                          isDarkTheme ? 'text-surface-0' : 'text-surface-900',
                          'font-bold text-lg',
                        ]"
                      >
                        {{ section.name }}
                      ...
                      >
                        {{ getEnabledPermissionsCount(section) }}/{{
                          getViewsForSection(section).length
                        }}
                        permisos activos
                      </p>
                    </div>
                  ...
                  >
                    <div class="flex items-center gap-4 flex-1 min-w-0">
                      <i
                        :class="[
                          view.icon,
                          isDarkTheme ? 'text-surface-400' : 'text-surface-600',
                          'text-xl',
                        ]"
                      ></i>
                    ...
                    <ToggleSwitch
                      ...
                      @update:modelValue="
                        (val) => {
                          getReactivePermission(view.key).isEnabled = val;
                          syncLinkedPermissions(view.key, val);
                        }
                      "
        ...
        <div class="flex justify-between items-center">
          <div class="flex items-center gap-2 text-sm text-surface-600">
            ...
            <span class="font-medium"
              >Los cambios se aplicarán inmediatamente al guardar</span
            >
          </div>
          <div class="flex gap-2">
            <Button
